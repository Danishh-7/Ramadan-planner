    ...
    return (
        <div className="space-y-10 animate-fade-in pb-12 font-serif">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <Card glass className="p-8 rounded-[3rem] notebook-border space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {prayerList.map((prayer) => {
                            ...
                            // Simplified Locking: Unlock Past, Current, and Next 5 Days
                            const isFuture = selectedDay > currentDay;
                            const isLocked = selectedDay > (currentDay + 5);
                ...
                {/* Color Legend */}
                <div className="flex flex-wrap items-center justify-center gap-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-completed"></div>Completed</div>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-missed"></div>Missed</div>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-qadha"></div>Qadha</div>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-muted/30 border border-border"></div>Not Started</div>
                </div>
