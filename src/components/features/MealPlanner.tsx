...
export const MealPlanner: React.FC = () => {
    ...
    return (
        <div className="space-y-12 animate-fade-in pb-12 font-serif">
            ...
            <div className="flex items-center justify-center gap-8 bg-background p-6 rounded-[2.5rem] notebook-border mx-auto max-w-sm shadow-inner">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <Card className="p-8 rounded-[3rem] notebook-border space-y-6 bg-card shadow-xl">
                    <div className="flex items-center justify-between border-b-2 border-dotted border-border pb-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Pre-Dawn</span>
                    </div>
                    <textarea
                        ...
                        className="w-full h-48 bg-transparent border-none focus:outline-none font-bold text-xl placeholder:text-muted-foreground/30 resize-none leading-relaxed text-foreground placeholder:italic"
                    />
                </Card>

                <Card className="p-8 rounded-[3rem] notebook-border space-y-6 bg-secondary/10 border-secondary/40 shadow-xl">
                    <div className="flex items-center justify-between border-b-2 border-dotted border-secondary/30 pb-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Sunset</span>
                    </div>
                    <textarea
            ...
            <div className="space-y-8">
                <div className="flex items-center gap-4">
                    <h3 className="text-3xl font-black italic text-foreground">Pantry Wisdom</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {[
                        ...
                        <Card key={idx} className="p-8 rounded-[2.5rem] notebook-border relative group overflow-hidden bg-card shadow-lg">
                            <span className="text-4xl absolute top-6 right-6 opacity-20 group-hover:scale-110 transition-transform">{tip.icon}</span>
                            <h4 className="text-xl font-black mb-4 text-card-foreground">{tip.title}</h4>
            ...
            <div className="space-y-8">
                <div className="flex items-center gap-4">
                    <h3 className="text-3xl font-black italic text-foreground">Sunnah Meal Options</h3>
                    <div className="h-px flex-1 bg-border/40" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                        { name: 'Talbina', desc: 'Barley flour cooked with milk and honey. Soothes the heart and relieves sadness.', ingredients: ['Barley Flour', 'Milk', 'Honey'] },
                        { name: 'Nabeez', desc: 'Soaked dates or raisins in water overnight. An alkalizing tonic for energy.', ingredients: ['Dates/Raisins', 'Water'] },
                        { name: 'Dates & Cucumber', desc: 'The Prophet (ﷺ) used to eat fresh dates with cucumber to balance heat.', ingredients: ['Fresh Dates', 'Cucumber'] },
                        { name: 'Honey & Water', desc: 'Warm water with dissolved honey on an empty stomach.', ingredients: ['Honey', 'Warm Water'] },
                        { name: 'Olive Oil', desc: 'Consume olive oil and anoint yourselves with it, for it is from a blessed tree.', ingredients: ['Extra Virgin Olive Oil'] },
                        { name: 'Barley Bread', desc: 'Whole grain barley bread, high in fiber and sustained energy.', ingredients: ['Barley Flour', 'Salt', 'Water'] }
                    ].map((item, idx) => (
